...
// ЭТП5-4: правило жетона доступа к чужой очереди — общее с телефоном. Здесь, в
...
const RELAY_CAPABILITIES = Object.freeze([
  ...
  'cover-traffic-v1',
  // ЭТП5-4: узел принимает жетон доступа и отдаёт по нему чужую очередь
  // предъявителю (`gw-pull`).
  //
  // Объявление здесь — условие работы всей затеи, а не запись для порядка. Узел
  // прежней версии на `gw-pull` НЕ ОТВЕЧАЕТ ВОВСЕ, и по молчанию шлюз не
  // отличит «не умею» от «ещё выгружаю»: он держал бы поездку до срока, а
  // человек без интернета всё это время ждал бы конверты, за которыми никто не
