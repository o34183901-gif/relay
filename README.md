...
сервис **watchtower** раз в 5 минут проверяет ghcr и сам пересоздаёт контейнер,
когда выходит новая версия (данные в volume сохраняются). Значит **обновить
тысячи релеев = один раз опубликовать новый образ** (push в этот репозиторий →
CI собирает и пушит образ → все релеи подтягивают его сами).

**Разовая настройка** (нужна, чтобы образ тянулся без логина):
1. Сделайте пакет образа публичным: GitHub → ваш профиль → **Packages** →
   `relay` → **Package settings** → **Change visibility** → **Public**.
2. На сервере: `cd relay && git pull && docker compose up -d`.

После этого обновления приходят автоматически. Проверить, что watchtower работает:
