...
## Пуши при закрытом приложении (FCM и UnifiedPush)

Релей умеет два механизма пробуждения: **FCM** (Firebase, устройства с
Google-сервисами) и **UnifiedPush** (web-push/VAPID — открытый push без Google,
для F-Droid / de-Googled устройств). Тип определяется автоматически по тому,
что зарегистрировало приложение: FCM-токен или web-push подписка.
...
### UnifiedPush (web-push / VAPID)

Работает «из коробки», настраивать ничего не нужно: при первом старте релей сам
генерирует VAPID-пару и сохраняет её в томе (`/data/vapid.json`, 0600).
Публичный ключ приложение получает от релея при подключении (кадр `ready`),
регистрирует подписку у своего UnifiedPush-дистрибьютора (например, ntfy) и
передаёт её релею. Пуш уходит зашифрованным web-push (RFC 8291).

Пару можно задать и явно — `RELAY_VAPID_PUBLIC`/`RELAY_VAPID_PRIVATE` (плюс
необязательный `RELAY_VAPID_SUBJECT`). Если у вас **несколько релеев**, ставьте
на все **одну и ту же** пару: подписка, сделанная под ключ релея A, не примется
релеем B (как и с общим FCM service-account).

Приватность: пуш (и FCM, и UnifiedPush) не содержит ни текста, ни имени
отправителя — только сигнал «есть новое сообщение».
...
он **проверяет cosign-подпись** образа (provenance: собран нашим CI, workflow
`image.yml` на `main`) и только при валидной подписи поднимает новый образ.
Значит **обновить тысячи релеев = один промоушен**: workflow сборки (Actions →
Run workflow) собирает, смоук-тестирует, подписывает и публикует образ в
`:canary`; после обкатки workflow промоушена переставляет `:stable` — и релеи
сами проверяют подпись и подтягивают образ. Контейнера с доступом к
...
- Промоушен/откат — workflow **«Промоушен/откат образа релея (canary ↔ stable)»**
...
| `RELAY_GOSSIP_TOKEN` | — | Общий токен push-gossip (`POST /relays`) между вашими релеями; без него анонимный HTTP-gossip выключен |
| `PORT` | `8787` | Порт релея |
| `RELAY_DB` | `/data/relay.db` (Docker); рядом с `relay.js` (bare-metal) | SQLite-БД релея: очередь, каталог, push-токены |
...
| `RELAY_METRICS_TOKEN` | — | Если задан, `/metrics` доступен извне по токену; без него — только из приватной сети/localhost |
| `TURN_HOST` | — | Публичный IP сервера — TURN для звонков |
| `TURN_SECRET` | автогенерация в `/data` | Необязательный override; по умолчанию релей сам генерирует секрет и хранит в томе |
| `FCM_PROJECT_ID` + `GOOGLE_APPLICATION_CREDENTIALS` | автопоиск `service-account.json` в `/data` | Пуши FCM; не нужны, если файл лежит в каталоге данных |
| `RELAY_VAPID_PUBLIC`/`RELAY_VAPID_PRIVATE` | автогенерация в `/data/vapid.json` | VAPID-пара для web-push (UnifiedPush); во флоте — одна пара на все релеи |
| `RELAY_VAPID_SUBJECT` | `mailto:relay@licno.invalid` | Контакт (subject) в VAPID-заголовке web-push |
