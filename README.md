...
## `bootstrap-relays.json` — аварийный вход в сеть

Приложение носит в сборке три адреса релеев. Если все три окажутся недоступны —
сменились адреса, режет провайдер, — восстановить связь ему нечем: у свежей
установки нет ни контактов, ни сохранённого пула. Чинилось бы это только новой
сборкой.

Поэтому в приложении есть второй путь: HTTPS-документ со списком адресов. Он
лежит **здесь**, а не на релее, и это главное в нём: документ, лежащий на релее,
недоступен ровно тогда, когда он нужен.

```
https://raw.githubusercontent.com/o34183901-gif/relay/main/bootstrap-relays.json
```

Формат — тот же, что отдаёт сам релей по `GET /relays`:

```json
{ "relays": ["wss://89.108.83.230.sslip.io", "..."] }
```

Ничего секретного в нём нет: эти адреса и так публикует каждый узел. Держите
список в согласии с действительностью — приложение обращается к нему только
тогда, когда не смогло подключиться ни к одному известному релею.

Имя намеренно НЕ `relays.json`: так называется рантайм-файл самого узла (он в
`.gitignore`), и совпадение имён путало бы каждого, кто развернул релей из этого
репозитория.
